...
public class LabelsView extends ViewGroup implements View.OnClickListener {
  ...
  private void getAttrs(Context context, AttributeSet attrs) {
    if (attrs != null) {
      ...
      mAddVisible = mTypedArray.getBoolean(R.styleable.LabelsView_AddLabelVisible, false);
      mTypedArray.recycle();
    }

    mAddTextView = new TextView(getContext());
    mAddTextView.setText("+添加标签");
  }
  @Override
  ...
  private void setAddTextViewStyle() {
    mAddTextView.setPadding(mTextPaddingLeft, mTextPaddingTop, mTextPaddingRight, mTextPaddingBottom);
    mAddTextView.setTextSize(TypedValue.COMPLEX_UNIT_PX, mTextSize);
    mAddTextView.setTextColor(mTextColor != null ? mTextColor : ColorStateList.valueOf(0xFF000000));
    //设置给label的背景(Drawable)是一个Drawable对象的拷贝，
    // 因为如果所有的标签都共用一个Drawable对象，会引起背景错乱。
    mAddTextView.setBackgroundDrawable(mLabelBg.getConstantState().newDrawable());
  }
  ...
  public void setAddTextViewListener(OnClickListener listener) {
    mAddTextView.setOnClickListener(listener);
  }
